...
def visualize_results(mesh, bed_history, bbox, resolution, history, T_physical, Ag, regime='fast', output_dir=None):
    ...
    pl(axes[1, 0], history.get('sediment_loss', []), 'Bed+Sed', 'g')
    pl(axes[1, 0], history.get('exner', []), 'Exner', 'olive', '--')
    pl(axes[1, 0], history.get('ic', []), 'IC', 'orange', '--')
    pl(axes[1, 0], history.get('transport_loss', []), 'C PDE', 'teal', ':')
    pl(axes[1, 0], history.get('gradation_loss', []), 'p PDE', 'brown', ':')
    axes[1, 0].set_title('Sediment Loss (Exner+C+p)')
    axes[1, 0].legend()
